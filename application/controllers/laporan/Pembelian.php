...
class Pembelian extends CI_Controller 
{
    ...
    public function get_report()
    {
        ...
        $data = $this->Transaksi_model->get_transaction_date($tanggal1, $tanggal2);
    ...
    public function cetak(){
        ...
        $data['ju'] = $this->Transaksi_model->get_transaction_date($tanggal1, $tanggal2);
        ...
        $pdf->Ln();
        $pdf->Cell(60 ,20,'',0,0);
        $pdf->Cell(59 ,5,'LAPORAN PEMBELIAN',0,0);
        $pdf->Cell(60 ,10,'',0,1);
        ...
        $pdf->Cell(50 ,8,'No Nota',1,0,'C');
        $pdf->Cell(60 ,8,'Nama Supplier',1,0,'C');
        $pdf->Cell(50 ,8,'Total Transaksi',1,0,'C');
        ...
        foreach ($data['ju'] as $d) {
            ...
            $cellHeight=8; //tinggi sel satu baris normal
                ...
                $textLength=strlen($d['nama_supplier']);	//total panjang teks
                ...
                while($startChar < $textLength){ //perulangan sampai akhir teks
                    ...
                    ($startChar+$maxChar) < $textLength ) {
                        $maxChar++;
                        $tmpString=substr($d['nama_supplier'],$startChar,$maxChar);
                    }
                ...
                $pdf->Cell(30 ,($line * $cellHeight),date('d M Y', strtotime($d['tanggal_pembelian'])),1,0,'L');
                $pdf->Cell(50 ,($line * $cellHeight),$d['no_nota'],1,0);
            ...
            $xPos=$pdf->GetX();
            $yPos=$pdf->GetY();
            $pdf->MultiCell($cellWidth,$cellHeight,$d['nama_supplier'],1);
            
            //kembalikan posisi untuk sel berikutnya di samping MultiCell 
            //dan offset x dengan lebar MultiCell
            $pdf->SetXY($xPos + $cellWidth , $yPos);

            $total_pembelian = $d['subtotal']+$d['total_pajak'];

            $pdf->Cell(50 ,($line * $cellHeight),'Rp '.number_format($total_pembelian, 0, '', '.'),1,1,'L');
        ...
        $pdf->Cell(50 ,8,'Rp '.number_format($total,0,'','.'),1,0,'L');
        $pdf->Output();
